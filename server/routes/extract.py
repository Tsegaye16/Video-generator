...
async def extract_content(request: ExtractRequest):
            ...
            for shape in slide.shapes:
                ...
                if shape.shape_type == MSO_SHAPE_TYPE.PICTURE:
                    ...
                    with open(img_save_path, 'wb') as f:
                        f.write(image.blob)
                    current_slide_data.images.append(ImageInfo(filename=img_filename, content_type=image.content_type, width_emu=image.size[0], height_emu=image.size[1]))
                    image_index += 1
    ...
    return ExtractionResponse(file_id=file_id, extracted_content_path=specific_extracted_path, slides=extracted_slides_data)
